...
// ---------------------------------------------------------------------------
static void robust_remesh(SurfaceMesh &m, double L, int n_iter, bool protect,
                          bool relax, bool verbose) {
  std::set<SurfaceMesh::Edge_index> border = collect_border_edges(m);
  PMP::isotropic_remeshing(
      faces(m), L, m,
      CGAL::parameters::number_of_iterations(n_iter)
          .edge_is_constrained_map(CGAL::make_boolean_property_map(border))
...
NumpyMesh weld_meshes(const std::vector<NumpyMesh> &meshes,
                      ...
                      int remesh_iterations, bool protect_constraints,
                      bool relax_constraints, bool verbose) {
  if (meshes.empty())
  ...
  for (std::size_t i = 1; i < meshes.size(); ++i) {
    if (verbose)
      std::cout << "[weld] === mesh " << i + 1 << " / " << meshes.size()
                << " ===\n";
    ...
    // after you append 'nxt' into 'out' and before merge_duplicated_vertices…
    std::vector<SurfaceMesh::Halfedge_index> borders;
    for (auto h : halfedges(out))
      if (is_border(h, out))
        borders.push_back(h);

    // add reversed duplicates so h and opposite(h) are both present
    const std::size_t n = borders.size();
    borders.reserve(2 * n);
    for (std::size_t i = 0; i < n; ++i)
      borders.push_back(opposite(borders[i], out));

    //        PMP::stitch_borders(borders, out);                 // orientation
    //        no longer matters :contentReference[oaicite:3]{index=3}
    // weld borders irrespective of orientation, accept ≤ eps deviation
    const double eps = duplicate_vertex_threshold; // e.g. 1e‑6 m
    PMP::stitch_borders(borders, out, CGAL::parameters::maximum_distance(eps));
    //        PMP::merge_duplicated_vertices_in_boundary_cycles(out);  //
    //        collapse double verts :contentReference[oaicite:4]{index=4}
